...
static void init_array_for(int ni, int nj, int nk, int nl,
                           double *alpha,
                           double *beta,
                           double A[ni][nk],
                           double B[nj][nk],
...
static void kernel_2mm_for(int ni, int nj, int nk, int nl,
                           double alpha,
                           double beta,
                           double tmp[ni][nj],
                           double A[ni][nk],
...
static void init_array_block_for(int ni, int nj, int nk, int nl,
		                           double *alpha,
		                           double *beta,
		                           double tmp[ni][nj],
		                           double A[ni][nk],
...
void mulbl_for(int ni, int nj, int nk, double A[ni][nj], double B[nk][nj], double C[ni][nk], double val)
{
  const int bs = BLOCK_SIZE;

  double a[bs * bs], b[bs * bs], c[bs * bs];

 #pragma omp for schedule(auto) private(a, b, c)
...
static void kernel_2mm_block_for(int ni, int nj, int nk, int nl,
	                              double alpha,
	                              double beta,
	                              double tmp[ni][nj],
	                              double A[ni][nk],
...
#pragma omp parallel shared(ni, nj, nl, nk, tmp, A, B, C, D, alpha, beta)
	{
		...
		mulbl_for(ni, nk, nj, A, B, tmp, alpha);
		...
		mulbl_for(ni, nj, nl, tmp, C, D, 1.0);
